...
if __name__ == "__main__":
    ...
    block_matrices_shape = jax.ShapeDtypeStruct(shape=(n_elems, n_per_e, n_per_e), dtype=jnp.float32)

    matvec_compiled = poisson_utils.preprocess.compile_for_inputs(block_assembl_poisson.poisson.blocked_matvec, (0,), p_order, block_matrices_shape, e_to_n, e_to_n_sorted, e_to_n_sort_idx, u_exact, bdry_indices)

    print(f"order : {p_order}\t est. (block-mat) matvec cost: {matvec_compiled.cost_analysis()['flops']}")


    print("compilation started..")

    nvtx.push_range("solver_compiling")
    solve_poisson_compiled = poisson_utils.preprocess.compile_for_inputs(block_assembl_poisson.poisson.solve_poisson, (0,), p_order, all_pts_gll_x, all_pts_gll_y, all_pts_gll_z, e_to_n, e_to_n_sorted, e_to_n_sort_idx, bdry_indices,
