...
import mat_free_poisson_v2.poisson
...
if __name__ == "__main__":
    ...
    KRON_GEMM_SHARED_LIBRARY = os.path.join(os.path.dirname(__file__), "mat_free_poisson_v2/kron_gemm_kernel/build/install/libkron_gemm_kernel.so")
    gemm_library = ctypes.cdll.LoadLibrary(KRON_GEMM_SHARED_LIBRARY)

    jax.ffi.register_ffi_target("IIA_kernel", jax.ffi.pycapsule(gemm_library.IIA_kernel),
                                platform="CUDA")
    
    jax.ffi.register_ffi_target("IAI_kernel", jax.ffi.pycapsule(gemm_library.IAI_kernel),
                                platform="CUDA")
    jax.ffi.register_ffi_target("AII_kernel", jax.ffi.pycapsule(gemm_library.AII_kernel),
                                platform="CUDA")
    ...
    matvec_compiled = poisson_utils.preprocess.compile_for_inputs(mat_free_poisson_v2.poisson.mat_free_matvec, (0,), 
                                                                  p_order, 
    ...
    solve_poisson_compiled = poisson_utils.preprocess.compile_for_inputs(mat_free_poisson_v2.poisson.solve_poisson, (0,), 
                                                                         p_order, 
                                                                         all_pts_gll_x, all_pts_gll_y, all_pts_gll_z, 
