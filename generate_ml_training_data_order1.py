...
def plot_random_samples(inputs, outputs, signal_infos, n_plots=8, save_dir='ml_data'):
    ...
    for plot_idx, sample_idx in enumerate(random_indices):
        ...
        if info['type'] == 'single':
            ...
            title_str = f"Gaussian (σ={info['std']:.2f})"
        elif info['type'] == 'polynomial':
            title_str = f"Poly (deg={info['degree']})"
        elif info['type'] == 'exponential':
            title_str = f"Exp (α={info['alpha']:.2f})"
        elif info['type'] == 'mixed':
            title_str = f"Mixed ({info['n_components']} comp)"
        elif info['type'] == 'step':
            title_str = f"Step ({info['n_steps']} steps)"
        elif info['type'] == 'sawtooth':
            title_str = f"Sawtooth (f={info['freq']:.1f})"
        else:  # chirp or other
            title_str = f"chirp: {info.get('f0', 0):.1f}→{info.get('f1', 0):.1f} Hz" if 'f0' in info else f"Type: {info['type']}"
    ...
    plt.suptitle(f'Order 1 FEM: Mixed Signal Training Data Samples\n'
                 f'Total samples: {n_samples}, Signal length: {n_pts}',
                 fontsize=13, fontweight='bold', y=0.995)
    
    # Save plot
    plot_path = os.path.join(save_dir, 'order1_mixed_training_samples.png')
...
def plot_statistics(inputs, outputs, signal_infos, save_dir='ml_data'):
    ...
    plt.suptitle(f'Order 1 FEM: Mixed Signal Dataset Statistics\n'
                 f'{n_samples} Samples, Signal Length: {n_pts}',
                 fontsize=14, fontweight='bold')
    
    # Save plot
    plot_path = os.path.join(save_dir, 'order1_mixed_training_statistics.png')
...
def plot_spectral_comparison(inputs, outputs, signal_infos, n_samples=6, save_dir='ml_data'):
    ...
    plt.suptitle('Order 1 FEM: Spectral Analysis of Mixed Signal Training Samples',
                 fontsize=13, fontweight='bold')
    
    plt.tight_layout(rect=[0, 0, 1, 0.97])
    plot_path = os.path.join(save_dir, 'order1_mixed_spectral_samples.png')
    plt.savefig(plot_path, dpi=100, bbox_inches='tight')
    plt.close()
...
def create_gaussian_signal(n, seed=None):
    """Create a Gaussian random signal with zero boundaries"""
    if seed is not None:
        np.random.seed(seed)
    ...
    # Apply sin(πx) envelope to ensure zero boundaries
    ...
    info = {'type': 'gaussian', 'std': noise_std}
    return signal, info

def create_random_signal(n, signal_type='random', seed=None):
    """Create various types of random signals with zero boundaries"""
    if seed is not None:
        np.random.seed(seed)
    ...
    if signal_type == 'random':
        # Randomly choose type
        signal_type = np.random.choice(['polynomial', 'exponential', 'mixed', 'step', 'sawtooth'], 
                                      p=[0.25, 0.2, 0.25, 0.15, 0.15])
    
    if signal_type == 'polynomial':
        ...
        # Exponential with sinusoidal modulation
        alpha = np.random.uniform(-2, 2)
        beta = np.random.uniform(0.5, 3)
        signal = envelope * np.exp(alpha * x) * np.sin(beta * np.pi * x)
        ...
        signal = envelope * np.sum(components, axis=0)
        ...
        signal = envelope * (2 * (x * freq - np.floor(x * freq + 0.5)))
...
def generate_training_data(n_samples=1500, save_path='ml_training_data_order1_mixed.npz'):
    """Generate training data with mixed signal types (sinusoidal, Gaussian, random) for order 1 FEM"""
    ...
    print("=" * 70)
    print(f"Generating {n_samples} Mixed Signal Input-Output Pairs for Order 1 FEM")
    print("=" * 70)
    ...
    # Generate samples
    print(f"\n2. Generating {n_samples} mixed signal samples...")
    
    input_signals = []
    output_signals = []
    signal_infos = []
    
    # Define signal type distribution
    signal_type_distribution = {
        'sinusoidal': 0.5,   # 50% sinusoidal (various types)
        'gaussian': 0.25,    # 25% Gaussian random
        'random': 0.25       # 25% other random functions
    }
    
    # Calculate number of samples per type
    n_sine = int(n_samples * signal_type_distribution['sinusoidal'])
    n_gaussian = int(n_samples * signal_type_distribution['gaussian'])
    n_random = n_samples - n_sine - n_gaussian  # Remaining samples
    
    print(f"   Signal distribution:")
    print(f"     Sinusoidal: {n_sine} samples ({100*n_sine/n_samples:.1f}%)")
    print(f"     Gaussian: {n_gaussian} samples ({100*n_gaussian/n_samples:.1f}%)")
    print(f"     Random: {n_random} samples ({100*n_random/n_samples:.1f}%)")
    ...
    for i in tqdm(range(n_samples), desc="Generating samples"):
        # Determine signal type for this sample
        if i < n_sine:
            # Sinusoidal signals
            input_signal, info = create_sine_signal(n_pts, signal_type='random', seed=20000 + i)
        elif i < n_sine + n_gaussian:
            # Gaussian signals
            input_signal, info = create_gaussian_signal(n_pts, seed=30000 + i)
        else:
            # Other random signals
            input_signal, info = create_random_signal(n_pts, signal_type='random', seed=40000 + i)
    ...
    print(f"  - {os.path.join(save_dir, 'order1_mixed_training_samples.png')} (sample plots)")
    print(f"  - {os.path.join(save_dir, 'order1_mixed_training_statistics.png')} (statistics)")
    print(f"  - {os.path.join(save_dir, 'order1_mixed_spectral_samples.png')} (spectral analysis)")
...
def main():
    # Generate 1500 samples with mixed signal types
    output_dir = "/Users/tahmidawal/sparse2dense/0912-SpMV-CNN2D-SparseMatrices-nonorm/ml_data"
    os.makedirs(output_dir, exist_ok=True)
    
    save_path = os.path.join(output_dir, "order1_mixed_1500samples.npz")
    
    # Generate the data
    inputs, outputs, signal_infos, sparse_matrix = generate_training_data(n_samples=1500, save_path=save_path)
    
    print("\n" + "=" * 70)
    print("Ready for ML training with Order 1 mixed signal data!")
